...
function loadGame(loadgame) {
  ...
  if (game.dragonName !== undefined) document.getElementById("dragonNameDisplay").innerHTML = game.dragonName
...
function enableEditMode() {
  const display = document.getElementById("dragonNameDisplay");
  const input = document.getElementById("dragonNameBox");
  
  // Hide the display span and show the input field with the current name
  display.style.display = "none";
  input.style.display = "inline-block";
  input.value = display.innerHTML.trim();
  input.focus();
}
...
function saveDragonName() {
  ...
  // I just studied COMP2041 and can't wait to use all my new regex knowledge!
  // This removes all the HTML tags except for a few formatting ones, as a user
  // requested so they can name their dragon 'Drakthar <sup>3</sup>'
  name = name.replace(/<(?!\/?(b|i|u|em|strong|sup|sub)\b)[^>]+>/g, "");

  game.dragonName = name || "Unnamed dragon";

  // Save the input value to the display span, hide input and show display
  display.innerHTML = game.dragonName;
